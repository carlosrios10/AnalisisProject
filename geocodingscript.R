...
# get the input data
infile <- "input"
data <- read.csv(paste0('./', infile, '.csv'),sep = '\t')
...
addresses = c(data$lon,data$lat)
addresses = paste0(data$l, ", Ireland")
...
for (ii in seq(startindex, nrow(data))){
    print(paste("Working on index", ii, "of", nrow(data)))
    #query the google geocoder - this will pause here if we are over the limit.
    result = getGeoDetails(c(data[ii,c('lon','lat')])) 
    print(result$status)     
    result$index <- ii
