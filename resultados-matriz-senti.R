...
resultados_red_social <- read.csv(paste(path,'selVecinos/resultados_UserNY_Base_Red_Social_Zona_matrizSenti_discreta_2.csv',sep = ""))
resultados_red_social[,variablesNum] <- round(resultados_red_social[variablesNum],digits = 3)
str(resultados_red_social)
resultados_condado <- read.csv(paste(path,'selVecinos/resultados_UserNY_Red_Visitas_Condado_matrizSenti_discreta_2.csv',sep = ""))
resultados_condado[,variablesNum] <- round(resultados_condado[variablesNum],digits = 3)

resultados_red_visitas <- resultados_condado[grepl('K_FRIENDS_NIVEL',resultados_condado$TVecinos),]
resultados_red_visitas[,variablesNum] <- round(resultados_red_visitas[variablesNum],digits = 3)

resultados_zona <- resultados_red_social[resultados_red_social$TVecinos == 'K_NEIGHBORHOOD_ZONA',]
...
resultados_red_social = rbind(resultados_red_social[grepl('K_FRIENDS_NIVEL',resultados_red_social$TVecinos),],resultado_base)
...
pdf(paste(path,'selVecinos/figuras/sel_vecinos_resultados_red_social_ingles_matrizSenti_discreta_2.pdf',sep = ""))
ggplot(melt_resultados_red_social, aes(x=Nvecinos, y=value,fill=TVecinos)) +
    ...
    scale_linetype_manual(name  ="",values=c("solid","solid","solid","solid","solid","dotdash"),
                            breaks=c("K_NEIGHBORHOOD", 
                                     "K_FRIENDS_NIVEL_1",
                                     "K_FRIENDS_NIVEL_2",
                                     "K_FRIENDS_NIVEL_3",
...
pdf(paste(path,'selVecinos/figuras/sel_vecinos_resultados_red_visitas_ingles_matrizSenti_discreta_2.pdf',sep = ""))
...
resultados_condado = resultados_condado[!grepl('K_FRIENDS_NIVEL',resultados_condado$TVecinos),]
...
#setEPS()
#postscript(paste(path,"figuras/neighbor_weighting_overlapliked_visitas2.eps"))
pdf(paste(path,'selVecinos/figuras/sel_vecinos_resultados_geo_ingles_matrizSenti_discreta_2.pdf',sep = ""))
ggplot(melt_resultados_condado, aes(x=Nvecinos, y=value,fill=TVecinos)) +
    ...
    expand_limits(y=c(0.5,0.9)) +
    scale_x_continuous(breaks=c(5,10,20,30,50))+ 
    scale_shape_manual(name  ="",values=c(17,15,1,18),
                         breaks=c("K_NEIGHBORHOOD",
    ...
    scale_linetype_manual(name  ="", values=c("dotdash","solid","solid","solid"),
                          breaks=c("K_NEIGHBORHOOD",
                                   "K_NEIGHBORHOOD_GRUPO_NIVEL_2", 
                                   "K_NEIGHBORHOOD_GRUPO",
                                   "K_NEIGHBORHOOD_ZONA"
...
dev.off()
###    
pdf(paste(path,'selVecinos/figuras/cantidad_vecinos_potenciales_ingles.pdf',sep = ""))
...
# Resultados coverage -----------------------------------------------------
34979.0
34745 34674 34497 34309 33948
34741 34670 34463 34315 34049
34694 34670 34590 34550 34514
34753 
